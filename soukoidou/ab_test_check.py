...
class ABTestCheck:
    ...
    def __init__(self, fetchKoitoKensa: IFetchData,
        ...
        self.path = r'\\192.168.1.247\共有\技術課ﾌｫﾙﾀﾞ\品質検査\小糸B試験管理ｼｰﾄ.xlsx'
        if platform.system() == 'Linux':
            self.path = r'/mnt/public/技術課ﾌｫﾙﾀﾞ/品質検査/小糸B試験管理ｼｰﾄ.xlsx'

        self.ab_check_df = pd.read_excel(self.path, sheet_name='matome', skiprows=4)
        hinbanTaiou_df = pd.read_excel(self.path, sheet_name='hinbantaiou')
        # hinbans = {'S4-K706CLA-U': 'K706A', ......}
        # hinmeis = {'K706改C':['S4-K706CLKC-U', 'S4-K706CLKC-4-U']....}を作る
        self.hinbans: Dict = dict(zip(hinbanTaiou_df['hinban'], 
                                                    hinbanTaiou_df['hinmei']))
        self.hinmeis: Dict = self.make_hinmeis(hinbanTaiou_df[['hinban', 'hinmei']])
        ...
        # 今回の小糸検査(済でない合格品)
        self.passed_koitos_thistime = cast(pd.DataFrame, 
                      passed_koitos_all[passed_koitos_all['IDOU']!='02'].copy())

        if not self.passed_koitos_thistime.empty:
        # 済の小糸検査合格品
            self.passed_koitos_sumi = cast(pd.DataFrame, 
                      passed_koitos_all[passed_koitos_all['IDOU']=='02'].copy())
        # 前回lotを探して入力
            self.passed_koitos_thistime['lastLot'] = \
            self.passed_koitos_thistime.apply(self.find_koito_lastLot, axis=1)
        # 前回lotのcountを入力
            self.passed_koitos_thistime['count_lastTime'] = \
       self.passed_koitos_thistime.apply(self.find_koito_lastLot_count, axis=1)
        
       

        '''
        1. 今回合格した小糸むけ製品 passed_koitos_thistime
        2. これまでの小糸向け製品 passed_koitos_all
        3. 前回ロットを探す passed_koito_last
        4. ab_check_dfで前回ロットが何番目かを求める
        5. 今回合格品のAB判定
        '''
    ...
    def check_is_abTest_ok(self)-> bool:
        ...
        # 空だったらabチェックはオッケー
        if self.passed_koitos_thistime.empty:
            txt = '<小糸AB試験チェック結果> \n' \
                  '~ 小糸AB試験はありません。~'
        ...
        As = [1, 2, 3, 5]
        Bs = [4]
        for _, row in self.passed_koitos_thistime.iterrows():
        ...
        self._recorder.out_log_df(self.passed_koitos_thistime,
                                 '<小糸AB試験チェック結果>')
        self._recorder.out_file_from_df(self.passed_koitos_thistime, 
                                  '<小糸AB試験チェック結果>')
    ...
    def find_koito_lastLot_count(self, row)-> int:
        ...
        lot = row['LOT']
        hinban =  row['Hinban']
        hinmei = self.hinbans[hinban] # K706改B etc
        lastLot = row['lastLot'][:10] # 詰め替え識別Noは除く
        # matching_indices = [0, 3, 10] など、lastLotと一致したindexのリストが返る 
        matching_indices = self.ab_check_df.index[self.ab_check_df[hinmei]
        ...
        count = self.ab_check_df.loc[index, '1～5'] 
    ...
    def find_koito_lastLot(self, row)-> str:
        ...
        hinbans = self.hinmeis[hinmei] # ['S4-K706KB-U', 'S4-K706KB-4-U'] etc
        # 前回lotを求める
        df = cast(pd.DataFrame, 
         self.passed_koitos_sumi[self.passed_koitos_sumi['Hinban'].isin(hinbans)])
        if not df.empty:
    ...
    def make_hinmeis(self, df):
        # hinmeiがhinmeisに無かったら新たにlist作って、あったらappendする 
